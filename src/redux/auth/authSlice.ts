...
>("auth/sendOtp", async (phone, { rejectWithValue }) => {
  try {
    ...
    // Firebase expects international format
    const formattedPhone = `+91${phone}`;

    await sendOtpService(formattedPhone);
...
>("auth/verifyOtp", async (otp, { getState, rejectWithValue }) => {
  try {
...
export const registerUser = createAsyncThunk<
  ...
  { state: { auth: AuthState }; rejectValue: string }
>("auth/registerUser", async (userData, { getState, rejectWithValue }) => {
  try {
